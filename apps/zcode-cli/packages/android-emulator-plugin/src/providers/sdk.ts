...
const TOOL_PATHS: Record<string, readonly string[]> = {
...
function sdkRoots(env: NodeJS.ProcessEnv = process.env): string[] {
    const configured = [env.ANDROID_PLUGIN_SDK_PATH, env.ANDROID_HOME, env.ANDROID_SDK_ROOT].filter((item): item is string => Boolean(item && item.trim()));
    return unique([
        ...configured,
...
function windowsSdkRoots(env: NodeJS.ProcessEnv): string[] {
    return [
