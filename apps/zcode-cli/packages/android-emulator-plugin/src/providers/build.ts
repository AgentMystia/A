...
import { type Run, brief, ok, run } from "../lib/run.js";
...
export async function buildApp(input: Build = {}): Promise<{
    ok: boolean;
    root: string;
    module: string;
    variant: string;
...
export async function buildAndRun(input: RunApp = {}): Promise<{
    installed: boolean;
    launched: boolean;
    ok: boolean;
    root: string;
...
async function projectRoot(raw?: string): Promise<string> {
    if (raw)
...
async function record(name: string, out: Run): Promise<string> {
