...
import { describe, keyevent, resolve, status, swipe, tap, typeText } from "../providers/ui.js";

const target = {
  serial: z.string().optional().describe(
    "Android device or emulator serial number. When omitted, the plugin uses a ready device/emulator or starts one when needed."
  ),
  avd: z.string().optional().describe("Android Virtual Device name, used when an emulator needs to be started."),
...
const buildInput = {
  ...
  applicationId: z.string().optional().describe("applicationId override to use when automatic discovery cannot resolve the app ID.")
};
...
server.registerTool(
  "android_preflight",
  {
    ...
    description: "Check whether the Android SDK, adb, emulator, AVDs, Java, Gradle project state, and ADB/UI Automator automation are available.",
    inputSchema: {}
  },
...
server.registerTool(
  "android_discover_project",
  {
    ...
    description: "Find the Gradle root, modules, variants, app ID, manifest, and APK outputs in the current project.",
    inputSchema: {}
  },
...
server.registerTool(
  "android_create_app",
  {
    ...
    description: "Create a minimal Kotlin and Jetpack Compose Android app in the current workspace.",
    inputSchema: {
      name: z.string().min(1),
      ...
      overwrite: z.boolean().optional().describe("Replace existing generated files. Defaults to false.")
    }
  },
...
server.registerTool(
  "android_build_and_run",
  {
    ...
    description: "Build the Android app, reuse a ready device/emulator or start a GUI emulator on demand, then install and launch the app.",
    inputSchema: {
      ...buildInput,
...
server.registerTool(
  "android_start_emulator",
  {
    ...
    description: "Start a new GUI Android emulator for the specified AVD. To reuse an existing device or emulator, pass its serial to build, install, launch, screenshot, log, or UI tools.",
...
server.registerTool(
  "android_create_avd",
  {
    ...
    description: "Create an Android Virtual Device with avdmanager. Confirm SDK package and license configuration with the user first.",
    inputSchema: {
...
server.registerTool(
  "android_screenshot",
  {
    ...
    description: "Capture a PNG screenshot from the selected Android device or emulator and return it as MCP image content.",
    inputSchema: {
      ...target,
...
server.registerTool(
  "android_ui_resolve",
  {
    ...
    description: "Resolve a text, content description, resource id, or class query to screen coordinates.",
    inputSchema: {
      ...target,
...
server.registerTool(
  "android_ui_keyevent",
  {
    ...
    description: "Press BACK, HOME, ENTER, APP_SWITCH, MENU, or SEARCH on the selected Android device.",
    inputSchema: {
      ...target,
...
async function guard(work: () => Promise<ReturnType<typeof json>>): Promise<ReturnType<typeof json>> {
  try {
    return await work();
  } catch (err) {
