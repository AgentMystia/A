...
const target = {
  udid: z.string().optional().describe(
...
const buildInput = {
  ...
  scheme: z.string().optional().describe("Xcode scheme. Automatically selected when it can be discovered."),
  configuration: z.string().optional().describe("Xcode configuration. Defaults to Debug."),
  derivedDataPath: z.string().optional().describe("DerivedData path as a relative path. Defaults to the plugin data directory."),
  bundleId: z.string().optional().describe("Override to use when build settings cannot resolve the bundle id."),
...
server.registerTool(
  ...
  {
    title: "iOS Preflight",
    description: "Check whether macOS, full Xcode, simctl, simulator runtimes, and optional UI backends are available.",
    inputSchema: {}
  },
...
server.registerTool(
  "ios_discover_project",
  {
    ...
    description: "Find .xcodeproj/.xcworkspace files, schemes, and bundle identifiers in the current project.",
    inputSchema: {}
  },
...
server.registerTool(
  "ios_screenshot",
  {
    ...
    description: "Capture a PNG screenshot from the selected simulator and return it as MCP image content.",
    inputSchema: {
      ...target,
...
server.registerTool(
  "ios_ui_status",
  {
    ...
    description: "Report whether the optional UI automation backend is available. P0 uses idb when available.",
    inputSchema: {}
  },
...
server.registerTool(
  "ios_ui_button",
  {
    ...
    description: "Press HOME, LOCK, SIDE_BUTTON, SIRI, or APPLE_PAY using the optional idb UI backend.",
    inputSchema: {
      ...target,
...
server.registerTool(
  "ios_ui_describe",
  {
    ...
    description: "Return accessibility information for the current screen using the optional idb UI backend.",
    inputSchema: target
  },
