...
export function parseSimulators(value: string): Sim[] {
    const raw = JSON.parse(value) as { devices?: Record<string, Array<Record<string, unknown>>> };
...
export async function show(udid?: string): Promise<{
    opened: boolean;
...
export async function install(udid: string, app: string): Promise<{
    installed: boolean;
    udid: string;
    app: string;
}> {
...
export async function launch(udid: string, bundle: string, args: string[] = []): Promise<{
    launched: boolean;
    udid: string;
    bundle: string;
    output: string;
...
export async function terminate(udid: string, bundle: string): Promise<{
    terminated: boolean;
    udid: string;
    bundle: string;
}> {
...
export async function url(udid: string, value: string): Promise<{
    opened: boolean;
    udid: string;
    url: string;
}> {
