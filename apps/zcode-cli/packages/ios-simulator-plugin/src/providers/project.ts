...
export async function discover(dir: string = process.cwd()): Promise<Found> {
    ...
    const source = workspaces[0] ? { flag: "-workspace" as const, file: workspaces[0] } : projects[0] ? { flag: "-project" as const, file: projects[0] } : undefined;
...
async function schemesFor(flag: "-project" | "-workspace", file: string): Promise<string[]> {
    ...
    if (!ok(out))
        return [];
    const raw = JSON.parse(out.stdout) as { project?: { schemes?: string[] }; workspace?: { schemes?: string[] } };
...
export async function bundle(flag: "-project" | "-workspace", file: string, scheme: string): Promise<string | undefined> {
...
function pick(input: { scheme?: string }, found: Found, file: string): string | undefined {
