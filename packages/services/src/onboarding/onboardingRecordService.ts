...
import {
  onboardingRecordDecisionSchema,
  ...
  return {
    ...
    async claimAnonymousRecord(): Promise<void> {
      ...
      await enqueueWrite(async () => {
        ...
        if (!file || hasIdentityRecord(file, userId)) return;
        ...
        for (let i = file.decisions.length - 1; i >= 0; i -= 1) {
          if (file.decisions[i]!.userId === null) {
            file.decisions[i] = onboardingRecordDecisionSchema.parse({
              ...file.decisions[i]!,
              userId,
            });
            await atomicWriteText(filePath, JSON.stringify(file, null, 2));
    ...
    async shouldOnboard(deviceMid: string): Promise<boolean> {
      const userId = await options.loadUserId();
      const existing = await readRecordFile(getRecordFile());
      if (existing && hasIdentityRecord(existing, userId)) return false;
      ...
      await enqueueWrite(async () => {
        ...
        file.decisions.push(
          onboardingRecordDecisionSchema.parse({
            userId,
    ...
    async dismissOnboarding(deviceMid: string): Promise<void> {
      ...
      await enqueueWrite(async () => {
        const filePath = getRecordFile();
        const file = (await readRecordFile(filePath)) ?? createFile(deviceMid);
        // 已有作答的身份不再写关闭决策；只覆盖还没有 entry 的 decision。
        if (file.entries.some((entry) => entry.userId === userId)) return;
        const decision = onboardingRecordDecisionSchema.parse({
          userId,
          status: "dismissed",
          reason: "user_closed",
          decidedAt: new Date().toISOString(),
