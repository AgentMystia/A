import type { MarketingTouchLocale } from "@zcode/shared";

export type BotMessageLocale = MarketingTouchLocale;
export type BotCopyKey = keyof typeof BOT_COPY["zh-CN"];
...
export const BOT_COPY = {
  "zh-CN": {
    ...
    "sessionExpiredNewTaskHint": "当前任务会话已失效，可能是任务已被清理或机器人消息已过期。请发送 **/new task** 创建新任务后再继续。",
    "deletedTaskReplaced": "原任务已删除，已为你新建任务。本条消息将在新任务中处理，不会继承原任务的对话上下文。",
    "received": "已收到。",
    "attachmentOnlyPrompt": "请查看附件并根据内容协助我。",
    "attachmentRejected": "附件处理失败：{message}",
    "attachmentDownloadUnavailable": "无法下载附件。文件可能已过期、已撤回，或机器人没有读取权限。请重新发送附件后再试。",
    ...
    "remoteDisconnected": "当前远端项目 {workspacePath} 未连接。请先发送 **/重连**，连接恢复后再重试。上一条请求未执行。",
    "remoteDisconnectedStatus": "当前远端项目 {workspacePath} 未连接。请发送 **/重连** 恢复连接。",
    "remoteWorkspaceSelectedDisconnected": "已切换到远端项目 {workspacePath}，但当前未连接。请先发送 **/重连** 后再执行任务。",
    "remoteReconnectStarting": "当前远端项目 {workspacePath} 未连接，正在为你重连...",
    "remoteReconnectFailed": "当前远端项目 {workspacePath} 重连失败：{message}\n上一条请求没有执行。",
    "remoteReconnectUnavailable": "当前远端项目 {workspacePath} 未连接，但机器人无法访问远端重连服务。请先在 ZCode 打开该远端项目后重试。",
  ...
  "en-US": {
    ...
    "userNotBound": "This bot is not bound. Generate a bind code in the zcode UI, then send **/bind <code>**.",
    ...
    "workspaceOutOfScope": "The workspace in this chat is no longer authorized. Please select **/workspace** again.",
    ...
    "weixinActivatedWelcome": "Weixin bot is active. Send **/help** to see commands, or describe what you want to do.",
    ...
    "sessionExpiredNewTaskHint": "The current task session has expired. It may have been cleaned up, or this bot message is stale. Send **/new task** to create a new task and continue.",
    "deletedTaskReplaced": "The previous task was deleted, so I created a new task for you. This message will be processed in the new task without the previous conversation history.",
    "received": "Received.",
    ...
    "attachmentDownloadUnavailable": "Could not download the attachment. The file may have expired, been removed, or the bot may not have permission to read it. Please send the attachment again and try once more.",
    ...
    "sessionModelUnavailable": "The session's model selection is unavailable. Use /model to choose again. Your saved selection has been preserved.",
    ...
    "elicitationMultiSelectHint": "You can select multiple options; select again to remove, then choose Done.",
    ...
    "taskRunning": "The current task is still running. Try again later, or use **/stop** to stop the current task.",
    "taskSelectTitle": "Current task {task}\nSelect task",
    "noHistoryTasks": "There are no history tasks in the current workspace.",
    "remoteDisconnected": "The remote workspace {workspacePath} is not connected. Send **/reconnect** first, then try again. The previous request was not executed.",
    "remoteDisconnectedStatus": "The remote workspace {workspacePath} is not connected. Send **/reconnect** to restore the connection.",
    "remoteWorkspaceSelectedDisconnected": "Switched to {workspacePath}, but the remote workspace is not connected. Send **/reconnect** before running tasks.",
    "remoteReconnectStarting": "The remote workspace {workspacePath} is not connected. Reconnecting now...",
    "remoteReconnectFailed": "Remote workspace {workspacePath} reconnect failed: {message}\nThe previous request was not executed.",
    "remoteReconnectUnavailable": "The remote workspace {workspacePath} is not connected, but the bot cannot access the remote reconnect service. Open this remote project in ZCode and try again.",
    "remoteReconnectLocal": "The current workspace is local and does not need reconnecting. Send **/workspace** to switch to a remote project.",
...
export function normalizeBotMessageLocale(locale: string | undefined): BotMessageLocale {
...
export function formatBotMessage(
  ...
  let text: string = BOT_COPY[normalizeBotMessageLocale(locale)][key];
