...
export function createWeixinChannelRuntime(options: {
  ...
  async function pollBot(bot: BotConfigEntry, signal: AbortSignal): Promise<void> {
    ...
    if (!token?.trim()) {
      options.statusSink.setRuntimeStatus({ botId: bot.id, provider: "weixin", status: "error", message: "Weixin bot token is missing." });
      return;
    }
    while (!signal.aborted) {
      ...
      try {
        ...
        await lock.release().catch((error) => {
          options.logger.debug(undefined, `release Weixin polling lock failed bot=${bot.id}: ${error instanceof Error ? error.message : String(error)}`);
        });
      }
    }
  }
  ...
  async function reconcile(config: BotsConfig | undefined, isCurrent: () => boolean): Promise<void> {
    ...
    const enabled = new Set(current.bots.filter((bot) => bot.provider === "weixin" && bot.enabled && bot.credentialRef).map((bot) => bot.id));
    ...
    scheduleRefresh(config) {
      if (options.runBackgroundTasks !== false) {
        refresh(config).catch((error) => {
          options.logger.warn(undefined, `refresh Weixin polling failed: ${error instanceof Error ? error.message : String(error)}`);
        });
      }
    },
    stopPolling,
