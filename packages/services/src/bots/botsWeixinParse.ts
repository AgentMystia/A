...
function inferWeixinAttachmentKind(item: Record<string, unknown>): string {
  ...
  const kind = `${readString(item, "kind")} ${readString(item, "media_type")} ${readString(item, "mime_type")} ${readString(item, "filename")}`.toLowerCase();
  if (kind.includes("image") || kind.includes("photo") || /\.(svg|png|jpe?g|gif|webp|heic|bmp)$/iu.test(kind)) {
    return "image";
  }
...
export function readWeixinText(message: Record<string, unknown>): string {
  const direct = readString(message, "text") || readString(message, "content") || readString(message, "message");
  if (direct) {
    return direct;
  }
  const nested = isRecord(message.msg) ? message.msg : isRecord(message.message) ? message.message : null;
  const items = Array.isArray(message.item_list)
    ? message.item_list
    : Array.isArray(nested?.item_list)
      ? nested.item_list
      : [];
  return items.map(readWeixinTextItem).filter(Boolean).join("\n") || readString(nested, "text") || readString(nested, "content");
}

function readWeixinAttachmentItem(
  item: unknown,
  index: number,
  ...
  const url = readString(merged, "url") || readString(merged, "download_url") || readString(merged, "downloadUrl");
  ...
  const size = readNumber(merged, "size") ?? readNumber(merged, "file_size") ?? readNumber(merged, "sizeBytes");
  const aesKey = readString(merged, "aes_key") || readString(merged, "aesKey") || readString(merged, "aeskey");
  return {
...
export function readWeixinMessageId(message: Record<string, unknown>): string | undefined {
  ...
  const numeric = readNumber(message, "id") ?? readNumber(message, "msgid") ?? readNumber(nested, "id");
...
export function readWeixinContextToken(message: Record<string, unknown>): string | undefined {
  const nested = isRecord(message.msg) ? message.msg : isRecord(message.message) ? message.message : null;
  return readString(message, "context_token") || readString(message, "contextToken") || readString(nested, "context_token") || undefined;
}

/** 发布包 host `buildInboundMessage`：message_type=2 视为非用户消息。 */
export function buildWeixinInboundMessage(botId: string, message: unknown): BotInboundMessage | null {
