import { isFeishuBotProvider, type BotConfigEntry, type BotsConfig, type BotProviderCallbackResult, type BotProviderOutbound } from "@zcode/shared";
...
export function createFeishuChannelRuntime(options: {
  ...
  async function runBot(bot: BotConfigEntry, signal: AbortSignal): Promise<void> {
    ...
    while (!signal.aborted) {
      ...
      try {
        ...
        socket = await startFeishuBotWebSocket({
          ...
          onConnectionStateChange: (state) => {
            options.statusSink.setRuntimeStatus({
              ...
              provider: bot.provider,
              status: "connected",
              message: state === "reconnecting" ? "Feishu WebSocket is reconnecting." : "Feishu WebSocket is running.",
              messageId: state === "reconnecting" ? "bots.runtime.feishuWebSocketConnecting" : "bots.runtime.feishuWebSocketRunning",
            });
          },
          ...
          onPayload: async (payload) => {
            ...
            options.logger.debug(undefined, `feishu websocket payload bot=${bot.id} ${options.summarizeCallbackPayload(payload)}`);
        ...
        await Promise.race([waitForAbort(signal), socket.terminated ?? new Promise(() => undefined)]);
      } catch (error) {
        if (signal.aborted) {
          return;
        }
        ...
        if (socket) {
          try {
            ...
            options.logger.debug(undefined, `close Feishu WebSocket failed bot=${bot.id}: ${error instanceof Error ? error.message : String(error)}`);
          }
          socket = null;
        }
        await lock.release().catch((error) => {
          options.logger.warn(undefined, `release Feishu WebSocket lock failed bot=${bot.id}: ${error instanceof Error ? error.message : String(error)}`);
        });
      }
      if (failed) {
        await waitFor(BOT_RUNTIME_ERROR_RETRY_MS, signal);
  ...
  async function reconcile(config: BotsConfig | undefined, isCurrent: () => boolean): Promise<void> {
    ...
    const enabled = new Set(
      current.bots.filter((bot) => isFeishuBotProvider(bot.provider) && bot.enabled && bot.credentialRef && bot.feishuAppId).map((bot) => bot.id),
    );
    ...
    for (const bot of current.bots) {
      if (isFeishuBotProvider(bot.provider) && bot.enabled && bot.credentialRef && bot.feishuAppId) {
        const credential = await options.credentialService.load(bot.credentialRef);
        if (!isCurrent()) {
          return;
        }
        const fingerprint = createBotConnectionFingerprint([bot.provider, bot.feishuAppId ?? "", bot.credentialRef ?? "", credential ?? ""]);
        const existing = sessions.get(bot.id);
        if (existing && existing.fingerprint !== fingerprint && (await stopWebSocket(bot.id), !isCurrent())) {
          return;
        }
    ...
    scheduleRefresh(config) {
      if (options.runBackgroundTasks !== false) {
        refresh(config).catch((error) => {
          options.logger.warn(undefined, `refresh Feishu WebSocket failed: ${error instanceof Error ? error.message : String(error)}`);
        });
      }
    },
    stopWebSocket,
