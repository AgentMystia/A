...
- `@zcode/shared` 的 `sideEffects` 是 `false`。tasks storage worker 引入 barrel 时，未使用的 `cloudContent` 与 `outputStyle` 不能进 host 共享 chunk。发布包把 `CLOUD_CONTENT_LOOPBACK_MIME` 和 `BUILTIN_OUTPUT_STYLES` 留在 host index。
