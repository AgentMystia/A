...
- `createDefaultBotsConfig`、`buildBotCredentialKey` 和 `buildBotWebhookSecretKey` 在 `botsDefaults.ts`，不和 `bots.ts` 的 schema 放在同一模块。schema、`isFeishuBotProvider` 与回复粒度仍留在 `bots.ts`。缺省 bot state 不是具名函数；`BotsRepo.readState` 在没有旧文件时直接使用 `{ version: 3, bots: {} }`。
...
- 手动领取 `server_time` 秒值乘 1000；JWT 键 `zcodejwttoken`；平台头 `${platform}-${arch}`。`readServerTimeMilliseconds` 从 `manualClaim.ts` 具名导出，不走 barrel `export *`。Host 里唯一运行时调用方是 coding-plan provider。
