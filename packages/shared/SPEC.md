...
- Host 的 tasks storage worker 和 Host index 共用 paths chunk。`storage-startup.ts` 引用 `initializeRuntimeProcessEnv`，但 worker 不调用它。这样 login shell、bundled tools 和 runtime PATH 补丁留在 paths chunk，不内联进 host index。引用本身没有运行时效果。`TaskIndexRepo` 在同一条初始化路径上、清理已删除分组引用之前，把 `acp_session_id` 与 `task_id` 不同的旧行迁到 ZCode session id。`migrateLegacyTaskMetaJson` 是具名函数。没有该列的库直接跳过。列探测是 `hasColumn(table, column)`，SQL 为 `PRAGMA table_info(${table})`，目前只传入常量 `tasks`。这段迁移只出现在 host paths chunk。`watchCronRunBotDelivery` 单独放在 `watchCronRunBotDelivery.ts`，只由 Host index 经 `@zcode/services/node` 引用，tasks storage worker 到不了它，所以它留在 host index。`getBotDeliveryTarget` 在方法体内 `safeParse(JSON.parse)`，失败走 `catch` 后返回 `undefined`。具名 `parseStoredBotDeliveryTarget` 仍给测试和 `@zcode/services/node`，`AutomationRepo` 不再引用它，host paths 和 scheduler 不留下这个 keepName。
...
- `migrateLegacyTaskMetaJson` 把 traceId 三元写进对象，先赋给 `let`，再 `zcodeTaskMetaSchema.parse`。直接写在参数里会被压成一次调用，host paths 少 8 字节。`listTaskMetas` 的 provider 过滤保留发布包 SQL 注释：`Bugfix: ZCode Agent 列表只接受当前 glm provider；Claude Code 导入也属于非 glm 历史数据。` `rowToMeta` 在 meta_json 无法解析时使用 `row.provider ?? undefined`，不再和 `ZCODE_AGENT_PROVIDER` 比较。
- `findZCodeAgentRuntimeBinary` 所在模块从 `node:path` 同时引入 `dirname` 和 `resolve`。函数不用 `dirname`；`void dirname` 只保住这条 import，压缩后 `void` 消失。host paths、main paths 和 scheduler 的 path import 因此是 `dirname` 加 `resolve`，不是只有 `resolve`。
- `TELEGRAM_ELSEWHERE_WAIT_MS`、锁持有 30 秒、租约触摸 10 秒和 `[100, 250, 500]` 定义在 `botsQueue.ts`，必须是 `export var`，并且排在 `createBotConnectionFingerprint` 前面。飞书、Telegram、微信三处“另一窗口已占用”的等待引用这份 10 秒绑定。锁实现和这三个指纹函数在同一个文件里，这样 host index 不会在 `var` 前再插入一次 `createHash` init。`botsLocks.ts` 只转出锁函数。`const` 会被折成多处字面量。发布包 host index 在指纹函数前只保留一条 `var td=1e4,ole=3e4,ile=1e4,sle=[100,250,500]`。
...
- broker catalog 随 `CuaHelperError` 保留一个未引用的 128MiB 上限 `PUBLISHED_UNUSED_BYTE_CEILING`。它不是 content bundle cache。发布包把它写成单独模块 `var publishedUnusedByteCeiling = 128 * 1024 * 1024`，前一个模块只 `void` 一次没有调用的 `execFileSync` 和 `platform`。这两份在 `helper-published-ceiling-imports.js` 和 `helper-published-byte-ceiling.js`，catalog 在临时文件残留之后、角色映射之前 side-effect 加载。不执行命令，也不是第二套安装器。16MiB 帧上限仍留在方法表旁边。
...
- `createDefaultBotsConfig`、`buildBotCredentialKey` 和 `buildBotWebhookSecretKey` 在 `botsDefaults.ts`，从 `@zcode/shared/botsDefaults` 导入，不从 barrel 再导出，也不和 `bots.ts` 的 schema 放在同一模块。schema、`isFeishuBotProvider` 与回复粒度仍留在 `bots.ts`。缺省 bot state 不是具名函数；`BotsRepo.readState` 在没有旧文件时直接使用 `{ version: 3, bots: {} }`。
...
- 手动领取 `server_time` 秒值乘 1000；JWT 键 `zcodejwttoken`；平台头 `${platform}-${arch}`。`readServerTimeMilliseconds` 从 `@zcode/shared/manualClaim` 导入，不从 barrel 再导出。Host 里唯一运行时调用方是 coding-plan provider。
...
- `bot_delivery_target` 只在 automation create 时写入。公开 automation 对象不携带它。唯一读取是 `AutomationRepo.getBotDeliveryTarget`，方法体内直接 `safeParse`；非法 JSON 或 schema 不匹配返回 `undefined`。`claimManualRuns` 的联表查询和交给 `rowToAutomation` 的行包含 `bot_delivery_target`；`rowToAutomation` 仍然丢掉该字段。`dispatchCronRun` 在 `trackCronRunOutcome` 与 `sendPrompt` 之前调用 `watchCronRunBotDelivery`；订阅失败只打日志，不取消派发。机器人消息把 `resolveAutomationBotDeliveryTarget` 放进 `sendPrompt`，session/send 与 v4 `sendText` 都带可选 `botDeliveryTarget`。没有 `chatType` 的目标不发送，因为发布包 schema 要求 `private` 或 `group`。
- `normalizeAgentProviderToZCodeAgent` 忽略入参，固定返回 `glm`。`ensureDraftOptions` 和 `writeDraftOptions` 经具名函数 `normalizeBotDraftOptions` 把草稿 provider 收成这个值；provider 变化时 `ensureDraftOptions` 写回 context。`botsNormalize.ts` 里同名的 schema 解析函数仍然存在，两份都是具名函数。
- 主任务 todo 投影的判定函数是 `isMainAgentToolProjectionSource`。`source=subagent`、`parentToolCallId`、`parentToolUseId`，以及 `_meta.claudeCode.parentToolUseId`（`readClaudeParentToolUseId`）都说明这条工具记录不属于主任务顶部摘要。
...
- `resumeSnapshotOrLegacy` 是 task adapter 唯一的 session 缺失恢复路径。先 `resumeSession`；只有 `Session not found` / `Session is not active` 才读 legacy snapshot。路径只取第一份已存在文件：`{taskId}.json`，否则 `{taskId}.deleted.json`。文件内容用 `readFileSync` 读成 utf-8 再 `JSON.parse`，不用 `fs/promises` 的 `readFile`。读失败或 schema 非法只打日志并返回 `null`，不改试下一份。`readLegacyTaskSnapshot` 合并 index meta 后把 task key 写入 adapter 内唯一的 legacy 集合；`migrationSource=claudeCode` 时尝试 `createSession(importedHistory)`，失败则继续只读。`resumeTask` 的 legacy 结果只 `syncTaskMeta` 并广播 `task_status_changed`。`getTaskSnapshot` 的 legacy 结果直接裁剪消息，index 已有 meta 时不再重写。该集合命中后 `onDynamicTaskEvent` 只订本地 emitter。两边都没有 mode 时按发布包写入 `"default"`；当前 mode 枚举不含该值，index 重读 `meta_json` 会告警并回退到 mode 列。
...
- `listWorkspaceRefs` 只在 workspace 缓存里声明一次。服务和 runtime 都写成 `listWorkspaceRefs: workspaceRefs.list`，不要在 `createInboundHandlers` 里再包同名函数。再包一层会让 host index 多一个 keepName，属性值也会再写出这个名字。
- `createProviderCallbackProcessor` 用立即执行的箭头返回工厂，不要写成函数声明。函数声明会在 host index 留下这个 keepName，发布包没有它。里面的 `processProviderCallback` 仍是函数声明。
- 发布包 host index 没有这些函数名时，导出函数用立即执行的箭头，不要写成函数声明：`deliverProviderCallbackReplies`、`handleStreamPermissionRequest`、`createInboundDeliveryDedupe`、`createTypingController`、`completeBotModelSelection`。`createTypingController` 里面的 `startTyping`、`stopTyping`、`stopInboundTyping` 仍是函数声明。
- 同样不要写成函数声明：`handlePublishedTaskStreamEvent`、`broadcastElicitationResolved`、`createWorkspaceRefCache`、`formatNumberedSelection`、`handlePermissionRespond`、`handlePermissionApprove`、`handlePermissionDeny`、`dispatchInboundMessage`、`handleBotReconnect`、`handleStopCommand`。`createWorkspaceRefCache` 里面的 `listWorkspaceRefs` 仍是函数声明。
- 发布包 host index 也没有这些函数名，写成立即执行的箭头：`elicitationStatus`、`isAckHandled`、`toProviderOutbound`、`writeBotPollingField`、`createBotPollingState`、`createBotProviderMap`、`createBotBindCode`、`toOutboundMessages`、`parseReplyGranularity`、`botTaskStreamKey`、`unwrapRegistrationData`、`readWebhookSecret`、`readFeishuCallbackToken`、`secretHeaderName`、`formatStatusModelSafe`。`createBotPollingState` 里面的 offset 与 buf 读写函数仍是函数声明。`createBotProviderMap` 里面的 `onDeliveryResult` 仍是函数声明。
- 发布包 host index 同样没有这些函数名：`toBotTaskProvider`、`createBotTraceId`、`readInboundAttachment`、`toPromptAttachment`、`readCardElicitation`、`isElicitationQuestion`、`readAnswerMap`、`readCardAnswerValues`、`readStatusProgressText`、`downloadFeishuAttachment`、`handleNewCommand`、`handleWorkspaceList`、`handleWorkspaceSet`、`handleThoughtLevelList`、`handleThoughtLevelSet`、`handleModelList`、`handleModelProviderSet`、`handleModelSet`、`handleTaskSet`、`createBotsMutationApi`、`createInboundHandlers`。这些写成立即执行的箭头。
- `@zcode/shared` 的 `sideEffects` 列出 `src` 里除 `src/pure` 以外的源码。整包写成 `false` 时，preload 会丢掉 barrel 里仍被发布包保留的模块。`CLOUD_CONTENT_LOOPBACK_MIME` 与 `BUILTIN_OUTPUT_STYLES` 不从 barrel 再导出；host 的内容服务和输出风格服务分别从 `@zcode/shared/cloud-content`、`@zcode/shared/output-style` 引入。barrel 只再导出这两份类型。发布包把这两份常量留在 host index，不放进 host 共享 chunk。
...
- 编译期本地开发标记和这份 build id 都是 `helper-install-plan.js` 里的模块级 `var`，不是函数。`readCompiledLocalDevelopmentRuntime` 与 `embeddedCuaHelperBuildId` 的函数声明会在 host index 和 main paths 各留下一次 keepName。发布包的标记是 `typeof __ZCODE_LOCAL_DEVELOPMENT_RUNTIME__ < "u"` 为真时取该值，否则取 `process.env.NODE_ENV !== "production"`。build id 只保留一个 `var`。写成 `const` 时，当前 esbuild 会把字符串内联进两个调用点，`pipeline-291748-cead36fd` 从 1 次变成 2 次。
...
- `helper-published-launch-bindings.js` 是启动模块前面的那组 node import。它导出 `execFile`、预检用的 `existsSync`、`readFileSync`、`rmSync` 和 `join`，另外 `void` 一次没有调用的 `randomUUID`、`writeFileSync`、`realpath`、`rm`、`basename`。这些 `void` 不读文件。`helper-launch.js` 的预检结果用这一份 `existsSync` / `readFileSync` / `rmSync`，`open` 用这一份 `execFile`。启动守卫仍用 `helper-launch.js` 自己的 `existsSync` 和 `realpathSync`。`resolveZcodeHome` 的 `join`、权限主体的 `execFile` / `realpath` / `basename` / `join` 也从这里来，不在各自文件里再写一份 node import。这不是第二套安装器。
...
- `helper-dev-broker.js` 把开发态凭据目录提成顶层 `join("computer-use", "run")`。`resolveDevBrokerCredentialsFilePath` 用这份目录再拼 `broker-credentials.json`，不把两段路径写进函数。调用 `join` 是 node import 的副作用：main paths 和 scheduler 摇掉函数后仍留下 `var x=join("computer-use","run")`，同时留下 `chmod`、`mkdir`、`rename`、`writeFile`。host 会调用函数，常量留在函数前面。
- broker 方法表在 `helper-broker-methods.js`，模块在 Set 处结束。`helper-published-frame-budget.js` 紧跟在它后面，用 `var` 留下 `16 * 1024 * 1024` 和 `8_000`。这两句不能写进 catalog，也不能写成 `const`：并进方法表会跟恢复码合成一条语句，`const` 会把后面的加法折成 `1e4`。`helper-reservation.js` 的 node import 顺序是 `fs/promises`、`net`、`path`、`process`。权限刷新宽限是这份 8 秒预算再加 `2_000`，`helper-host-termination.js` 读同一个绑定，不再写字面量 `10_000`。main paths 和 scheduler 摇掉预留函数后仍留下 `var x=eight+2e3` 和那组 node import。
...
- `BROKER_RECOVERY_MESSAGES` 之后，`broker.js` 按顺序 side-effect 加载 `helper-hold-duration.js`、`helper-published-peer-freeze.js`、`helper-published-connection.js`、`helper-published-async-local.js`、`helper-published-deadline-keys.js`。它们必须写在 `helper-broker-catalog.js` 的 import 之后，不能写进 catalog 文件内部，否则 esbuild 会把它们抬到方法表前面。这五份只跟随 broker 静态链，出现在 main 的 paths chunk、host index 和 scheduler index；不进 preload、renderer，也不进 helperHealth 的交换 chunk。`helper-hold-duration.js` 拥有 `boundedHoldDuration` 和 `businessResponseTimeout`。模块开头是空的 `if (0)`，然后是 `var publishedUnusedRefreshMs = 30 * 1e3` 与 `var publishedHoldSlackMs = 5 * 1e3`。纯常量赋值会被删掉，`if (0)` 让两句留下，`if` 压缩后消失。main 和 scheduler 不调用这两个函数，产物只剩 `var a=30*1e3,b=5*1e3`。host 会调用，所以两个函数留在这句后面，`businessResponseTimeout` 读 `publishedHoldSlackMs`，不写成字面量 `5e3`。按住上限在函数里仍是字面量 `30`，再乘 `1_000`。这个没有引用的 30 秒不是 `helper-refresh-marker.js` 里会写入的截止时间。peer 残留是 `helper-published-peer-freeze.js`：裸 `import "node:net"`，加上 `Object.freeze({ verifySocketPeer(socketPath, stat) {} })`。空方法不是 `permissionBrokerClient.js` 的 `defaultPeerCredentialChecker`，不要改真正的检查。下一份是 `helper-published-connection.js`，只有 `import { createConnection } from "node:net"` 和 `void createConnection`。`void` 在压缩后消失，import 留下来，因为绑定被读过一次。AsyncLocalStorage 残留是一次没有读取的 `new AsyncLocalStorage()`，压缩后仍从 `async_hooks` 导入并 `new`。截止字段残留在 `helper-published-deadline-keys.js`：先 `import { closeSync, fstatSync, lstatSync, openSync, readFileSync, readSync } from "node:fs"`，每个绑定 `void` 一次，再 `Object.freeze(new Set(["schema", "kind", "deadlineEpochMs"]))`。这些 `void` 不是读取文件；压缩后只剩 import 和 Set。真正写入 `deadlineEpochMs` 的仍是刷新标记。这些残留不是第二套安装器，也不是第二份 Host。
- 同一条 catalog side-effect 还保留发布包里未调用的 AX 常量表，不实现 flattener 函数。名字表是 `associateTitleUIElements`、`flattenIntoSelectableAncestor`、`pruneEmptyDisabledElements`、`mergeSingleItemGroups`、`flattenRepetitiveStaticText`、`flattenLinksIntoMarkdownText`。文本角色集含 `searchfield`；角色映射表把 `Button` 映到 `button`，`ListItem` / `DataItem` / `TreeItem` 映到 `row`，`Custom` / `Pane` / `Window` / `Group` 映到空字符串，其后的角色集不含 `searchfield`。按键集含 `forwarddelete` 与 `f1`–`f16`。键页是 `64` 与 `64*64`，单独写在 `helper-published-ax-page.js`，catalog 在按键集之后、临时文件残留之前 side-effect 加载它。写进 `helper-published-ax-tables.js` 会被并进 Set 那条语句，发不出 `var x=64,y=x*x`。角色映射和桌面 bundle id（`dev.zcode.app` 与 `dev.zcode.app.preview`）在 `helper-published-ax-roles.js`。原生绑定路径 `build/Release/ax_native.node` 在 `helper-published-ax-native.js`，同模块 `void` 没有调用的 `createRequire`、`existsSync`、`dirname`、`isAbsolute`、`fileURLToPath`。它后面、PiP schema 前面还有两份空 import：`helper-published-temp-fs.js`（`randomUUID`、`execFileSync`、`lstat`、`mkdir`、`open`、`readFile`、`rm`、`writeFile`、`join`、默认 `process`）、`helper-published-temp-require.js`（`createRequire`、`join`）。main 和 scheduler 里那份没有函数体的 `execFileSync` / `realpathSync` / `join` 是 `helper-process-evidence.js` 被摇掉函数后的残留，不是另一份 catalog 模块。host 上同一份 import 后面接着 `32` 和 `-awwxo`。`helper-published-temp-spawn.js` 在临时文件残留之后，`void` 没有调用的 `spawnSync`、`realpathSync`、`basename`、`dirname`、`isAbsolute`、`relative`、`resolve`、`sep`。这些 `void` 不读文件、不创建进程。catalog 在键页和角色映射之间 side-effect 加载三份没有函数体的临时文件残留：`helper-published-temp-scan.js`（`execFile`、`execFileSync`、`mkdtempSync`、`readFileSync`、`rmSync`、`tmpdir`、`basename`、`join`）、`helper-published-temp-read.js`（`execFile`、`readdir`、`readFile`、`homedir`、`basename`、`join`）、`helper-published-temp-limit.js`（同样的 mkdtemp import，但没有 `basename`，并留下没有读取的 `var publishedUnusedTempReadLimit = 16 * 1024 * 1024`）。每个绑定只 `void` 一次，不创建临时目录，也不读文件。`if (0)` 让 16MiB 常量留下，`if` 压缩后消失。这三份跟着 catalog，出现在 main paths、host index 和 scheduler index。这些值不是第二套安装器。
...
- `reapOrphanedHelpers` 只在 darwin 且能取到 uid 时扫描。它只对 ppid 为 1、可执行文件和随机 socket 都落在安装根里、launcher pid 已死或已变成非 ZCode 命令的 Helper 发 `SIGTERM`，单次最多 32 个。活着的 launcher 默认放过。这个 32 和 `ps -awwxo` 参数写在 `helper-process-evidence.js` 的同一条常量里。该文件只从 node 引入 `execFileSync`、`realpathSync`、`join`，并把 `realpathSync` 再导出给 reaper，避免 reaper 自己再写一份 `node:fs` import。`\.app$` 在可执行路径、单包路径和命令前缀三处各写一次，不抽成顶层函数。`@zcode/zcode-cua` 的 `package.json` `sideEffects` 列出包内每个会被 bare import 或未使用 export 留在包里的模块，包括 `helper-process-evidence.js`。不列出时，main 和 scheduler 会丢掉这组 import。新增这类文件时必须把该文件补进 `sideEffects`。
- 异步 `listProcesses`、`listUnixSocketOwnerPids` 和 `discoverCuaHelperLaunchProcesses` 在 `helper-live-identity.js`。这个模块从 node 引入 `execFile`、`existsSync`、`realpathSync`、`isAbsolute`、`relative`、`resolve`，Contents/MacOS 用 `resolve`。`listProcesses` 再内联一份 `-awwxo` 参数，并调用 `helper-exec-file-text.js` 的 `execFileText`。全仓库只有这一份 `execFileText` / `formatErrorMessage`：模块只 import `execFile`，失败信息用换行拼接 stderr。安装校验、解压和清 quarantine 也调用它，不再各写一份。`helper-install-verify.js` 不从 node 引入 `execFile`、`existsSync`、`join`；`existsSync` 和 `join` 复用 launch bindings，工具路径用 `helper-tools.js` 的 `HELPER_TOOLS`（plist 键是 `plist`）。`helper-install-archive.js` 的 unzip、ditto、xattr 也用这份表，不另写 `{ ditto, unzip, xattr }`。否则 main paths 和 host 会多出工具路径，并留下一个只有 `execFile` / `existsSync` / `join` 的空 import 模块。`parsePsProcessRows` 留在进程证据模块。候选路径和 `resolveHelperAppPath` 留在 `helper-install-plan.js`，`existsSync` 复用 launch bindings，不在安装计划里再写一份 `node:fs` import。
- `isScreenCaptureProbeSuccess` 只接受 `foreign_window` 且内容证据为 `decoded_visible_non_uniform` 的探针。窗口尺寸和采样像素对不上时返回 false。
- `readLegacyImportedClaudeHistory` 同步选择第一份已存在的 legacy snapshot：`existsSync` 判定 `{taskId}.json`，否则 `{taskId}.deleted.json`，再用 `readFileSync` 读取。schema 不是 `migrationSource=claudeCode` 时返回 `null`，不改试下一份。返回值不含 `traceId`。`resolveImportedClaudeHistoryForRepair` 不 await 这次同步读取；原 Claude jsonl 仍是异步读取。
- Provider 工作区配置目录只属于 `packages/services/src/paths.ts`。真正用到的 node 绑定从 `pathsPublishedNodeBindings.ts` 引入。那个文件另外保留发布包里没有调用的 `copyFileSync`、`existsSync`、`mkdirSync`、`readFileSync`、`unlinkSync`、`writeFileSync`，每个绑定只 `void` 一次，不读文件。`paths.ts` 自己再 `void` 一次 `readFileSync` 与 `writeFileSync`，让 paths 模块开头单独留下这两个 import。目录表记录 claude、opencode、gemini、codex、glm 的原生文件名，但不扩大 `ZCodeProvider`。`gemini` 落在隔离目录下的 `.gemini`；`glm` 使用数据根下的 runtime `nativeConfigDir`；其它 id 使用 `~/.zcode/v2/agent-config/<id>/<workspaceHash>`。Claude 历史导入的 projects 目录走这一个函数。

- 简单交换只属于 `packages/zcode-cua/broker-exchange.js` 的 `brokerExchange`。`CuaHelperError` 和 `isCuaHelperError` 也在这个文件里，`code` 是类字段。`broker.js` 再导出它们。`callBrokerMethod` / `probeHelperHealth` 走它：先发 `id:0` 的空 `authenticate`，再发 `id:1` 的业务方法。鉴权被拒是 `CuaHelperError`，`code` 为 `auth_failed`。`probeHelperHealth` 在截止时间前按 `broker_info` 轮询，超时 `code` 为 `health_timeout`。它写在 `callBrokerMethod` 前面。成功时先用 `let` 取出 `bundleId` 和 `pid` 再返回。超时文案直接内联 `lastError` 判断，不另起 `detail` 变量。helperHealth 同时转出 `brokerExchange`，不转出 `isCuaHelperError`。host 会调用这个判断，所以它留在交换 chunk；main 和 scheduler 没有调用。`brokerExchange` 接收一个参数再用 `let` 拆出 `socketPath`、`method`、`params`、`timeoutMs`。已经鉴权后直接 resolve；否则进入 `else`，先把标志设为真，再拒绝 `ok !== true`。参数解构或拿掉 `else` 时，main 和 host 的交换 chunk 各少 9 字节。`broker-exchange.js` side-effect 导入 `broker-helper-health.js`。没有这条回边时，main 的交换 chunk 少 8 字节，host 和 scheduler 各少 4 字节。发布包把第二次初始化放在 `net` import 之后；当前 esbuild 把两次调用都放在 import 前面，所以长度对齐了，字节顺序还没有。
- `PermissionBrokerClient` 只属于 `permissionBrokerClient.js`。每条 RPC 新建连接，鉴权帧带 `clientApiVersion:2` 与 `authenticateParams`，业务 id 从 1 递增。默认 peer 检查拒绝 world-writable socket；Windows 管道必须落在 `\\.\pipe\zcode-cua-helper-`。`hold_key` / `hold_key_to_app` 的等待时间是 `max(timeout, min(duration,30)*1000+publishedHoldSlackMs)`，其中 `publishedHoldSlackMs` 是 `helper-hold-duration.js` 里的 `5 * 1e3`。
- PiP 客户端只属于 `pip-session-node.js`。它排队调用 `PermissionBrokerClient`，角色 `presentation`，协议 `2` / `zcode-cua-pip-session-v2`。协议版本和 runtime id 只写在客户端里，所以只有 host 保留 `zcode-cua-pip-session-v2`。事件 schema 在 `pip-session-schema.js`，由 catalog side-effect 加载；main 的 paths chunk、host index、scheduler index 都留下这份 zod 对象，即使客户端被摇掉。`send` 调用 discriminated union 的 `.parse`，不另写 `parsePipSessionEvent`。schema 是 strict object：`focus-changed`、`turn-started`、`turn-ended`、`session-closed`。标识符 trim 后长度 1–255，不能含 NUL，也不能是 `__zcode_pip_no_active_session_v2__`。同一模块还保留未调用的窗口对象 schema，以及 `{ protocolVersion, runtimeId }` 和 `{ event }` 两个 strict object。握手不一致时记 `version_mismatch` 并不再发送。`broker_unavailable` 在 `reconnectAttempts` 内重试，其它错误立即抛出。诊断回调只发第一次。
...
- zip bundle zod 也只有一份，仍是 `cloudDialogPayload.ts` 里的 `cloudDialogBundleSchema`。`cloudContentBundleSchema` 导出同一个对象，内容租约和 interactive hero 都解析它。URL 最长 4096，必须是不带用户名和密码的 http(s)；`format` 为 `zip`，`entry` 1–240，`sha256` 为 64 位小写十六进制，可选 `sizeBytes` 不超过 8MiB。发布包 main、host、scheduler 和每份 preload 只出现一次 `literal("zip")`。不要在 `cloudContent.ts` 再构造一份：那个副作用对象会进 preload，并把 loopback CSP / MIME 从 host index 带进 host 共享 chunk。CSP 和 MIME 仍只写在 `cloudContent.ts`。
...
- `createMarketingAssetRegistry` 的下载上限写在 `download` 里面：图片 8MiB、视频 16MiB，表达式是 `(kind === "image" ? 8 : 16) * 1024 * 1024`。data URL 预算 48MiB 和 PNG 文件头也在函数体内。不要放在模块顶层。顶层 `Buffer.from` 会被 esbuild 当成副作用，main paths 和 scheduler 摇掉下载函数后仍留下 `8*1024*1024`、`16*1024*1024` 和 PNG 头。host 仍执行下载。
