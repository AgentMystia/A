...
- 同一条 catalog side-effect 还保留发布包里未调用的 AX 常量表，不实现 flattener 函数。名字表是 `associateTitleUIElements`、`flattenIntoSelectableAncestor`、`pruneEmptyDisabledElements`、`mergeSingleItemGroups`、`flattenRepetitiveStaticText`、`flattenLinksIntoMarkdownText`。文本角色集含 `searchfield`；角色映射表把 `Button` 映到 `button`，`ListItem` / `DataItem` / `TreeItem` 映到 `row`，`Custom` / `Pane` / `Window` / `Group` 映到空字符串，其后的角色集不含 `searchfield`。按键集含 `forwarddelete` 与 `f1`–`f16`。键页是 `64` 与 `64*64`。桌面 bundle id 冻结为 `dev.zcode.app` 与 `dev.zcode.app.preview`。原生绑定路径是 `build/Release/ax_native.node`。这些值不是第二套安装器。
