...
- `BROKER_RECOVERY_MESSAGES` 之后，`broker.js` 按顺序 side-effect 加载四份没有调用方的发布包残留：`helper-published-refresh-window.js`、`helper-published-peer-freeze.js`、`helper-published-async-local.js`、`helper-published-deadline-keys.js`。它们必须写在 `helper-broker-catalog.js` 的 import 之后，不能写进 catalog 文件内部，否则 esbuild 会把它们抬到方法表前面。这四份只跟随 broker 静态链，出现在 main 的 paths chunk、host index 和 scheduler index；不进 preload、renderer，也不进 helperHealth 的交换 chunk。刷新窗口是空的 `if (0)` 加上 `var publishedUnusedRefreshMs = 30 * 1e3` 与 `var publishedUnusedRefreshGraceMs = 5 * 1e3`。纯常量赋值会被删掉，`if (0)` 让两句留下，`if` 压缩后消失，产物是一句 `var a=30*1e3,b=5*1e3`。这不是 `helper-refresh-marker.js` 里会写入的 30 秒。peer 残留是 `Object.freeze({ verifySocketPeer(socketPath, stat) {} })`，空方法，不是 `permissionBrokerClient.js` 的 `defaultPeerCredentialChecker`，不要改真正的检查。AsyncLocalStorage 残留是一次没有读取的 `new AsyncLocalStorage()`，压缩后仍从 `async_hooks` 导入并 `new`。截止字段残留是 `Object.freeze(new Set(["schema", "kind", "deadlineEpochMs"]))`，没有读取方；真正写入 `deadlineEpochMs` 的仍是刷新标记。发布包同一段还有未使用的 `fs` / `net` import。当前 esbuild 会丢掉没有读取的 node 内置 import，不要用 `void` 或 `if (0)` 去留它们。这些残留不是第二套安装器，也不是第二份 Host。
