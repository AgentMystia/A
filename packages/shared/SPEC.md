...
- 端点解析以 `zcodeEndpoint.ts` 为唯一实现。未传 `env` 时 `resolveZCodeEndpointOrigin` 按测试通道处理：生产通道只用 `envBaseOrigin`，否则 `https://zcode.z.ai`，忽略 `overrideOrigin`；测试通道依次用 `overrideOrigin`、`envBaseOrigin`、`https://zcode.chatglm.site`。运行时通道只在 `ZCODE_ENV=test` 时为测试，否则为生产。通用键 `ZCODE_BASE_URL` / `ZCODE_ENDPOINT_ORIGIN` 优先于 `ZCODE_PRODUCTION_BASE_URL` 或 `ZCODE_TEST_BASE_URL`。BigModel 同样先读 `BIGMODEL_API_BASE_URL`，再读通道键，最后才是 `https://bigmodel.cn` 或 `https://dev.bigmodel.cn`。ZAI OAuth、business 与 client id 也按通道回落到 `https://chat.z.ai` / `https://zai-test.chatglm.site`、`https://api.z.ai` / `https://api.chatglm.site`，以及两个公开 client id。Coding Plan webview 固定信任 `https://zcode.z.ai`、`https://zcode.chatglm.site` 和 `http://localhost:3000`；其余地址只有 E2E loopback 才信任。
- 任务 mode 词表是 `default`、`yolo`、`plan`、`edit`、`acceptEdits`、`auto`、`dontAsk`、`bypassPermissions`、`autoEdit`、`build`。Relay task meta 的 provider 词表接受历史值 `claude`、`opencode`、`gemini`、`codex`、`glm`，但不扩大运行时 `ZCodeProvider`。持久化 task meta 另有可选 `repairState`，只记录 Claude / Codex 原生快照修复版本。
