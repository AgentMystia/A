...
- `BROKER_RECOVERY_MESSAGES` 之后，`broker.js` 按顺序 side-effect 加载 `helper-hold-duration.js`、`helper-published-peer-freeze.js`、`helper-published-async-local.js`、`helper-published-deadline-keys.js`。它们必须写在 `helper-broker-catalog.js` 的 import 之后，不能写进 catalog 文件内部，否则 esbuild 会把它们抬到方法表前面。这四份只跟随 broker 静态链，出现在 main 的 paths chunk、host index 和 scheduler index；不进 preload、renderer，也不进 helperHealth 的交换 chunk。`helper-hold-duration.js` 拥有 `boundedHoldDuration` 和 `businessResponseTimeout`。模块开头是空的 `if (0)`，然后是 `var publishedUnusedRefreshMs = 30 * 1e3` 与 `var publishedHoldSlackMs = 5 * 1e3`。纯常量赋值会被删掉，`if (0)` 让两句留下，`if` 压缩后消失。main 和 scheduler 不调用这两个函数，产物只剩 `var a=30*1e3,b=5*1e3`。host 会调用，所以两个函数留在这句后面，`businessResponseTimeout` 读 `publishedHoldSlackMs`，不写成字面量 `5e3`。按住上限在函数里仍是字面量 `30`，再乘 `1_000`。这个没有引用的 30 秒不是 `helper-refresh-marker.js` 里会写入的截止时间。peer 残留是 `Object.freeze({ verifySocketPeer(socketPath, stat) {} })`，空方法，不是 `permissionBrokerClient.js` 的 `defaultPeerCredentialChecker`，不要改真正的检查。AsyncLocalStorage 残留是一次没有读取的 `new AsyncLocalStorage()`，压缩后仍从 `async_hooks` 导入并 `new`。截止字段残留是 `Object.freeze(new Set(["schema", "kind", "deadlineEpochMs"]))`，没有读取方；真正写入 `deadlineEpochMs` 的仍是刷新标记。发布包同一段还有未使用的 `fs` / `net` import。当前 esbuild 会丢掉没有读取的 node 内置 import，不要用 `void` 或 `if (0)` 去留它们。这些残留不是第二套安装器，也不是第二份 Host。
