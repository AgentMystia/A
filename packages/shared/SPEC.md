...
- `TELEGRAM_ELSEWHERE_WAIT_MS`、锁持有 30 秒、租约触摸 10 秒和 `[100, 250, 500]` 定义在 `botsQueue.ts`，必须是 `export var`，并且排在 `createBotConnectionFingerprint` 前面。飞书、Telegram、微信三处“另一窗口已占用”的等待引用这份 10 秒绑定。`botsLocks.ts` 读取同一组绑定。`const` 会被折成多处字面量。发布包 host index 在指纹函数前只保留一条 `var td=1e4,ole=3e4,ile=1e4,sle=[100,250,500]`。
